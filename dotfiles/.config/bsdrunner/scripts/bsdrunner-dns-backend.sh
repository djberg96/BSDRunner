...
resolv_backup_file="$state_dir/resolv.conf.before-network-dns"
...
network_nameservers() {
    if command -v resolvconf >/dev/null 2>&1; then
        resolvconf -l 2>/dev/null | awk '
            $1 == "nameserver" && $2 != "" && $2 != "127.0.0.1" && $2 != "::1" {
                if (!seen[$2]++)
                    print $2
            }
        '
    ...
    [ -d /var/run/resolvconf/interfaces ] || return 0
    awk '
        $1 == "nameserver" && $2 != "" && $2 != "127.0.0.1" && $2 != "::1" {
            if (!seen[$2]++)
                print $2
        }
    ' /var/run/resolvconf/interfaces/* 2>/dev/null || true
}
...
network_nameservers_json() {
    first=1
    printf '['
    network_nameservers | while IFS= read -r server; do
        [ -n "$server" ] || continue
        if [ "$first" -eq 0 ]; then
            printf ','
        fi
...
write_network_resolv_conf() {
    tmp="$(mktemp "${TMPDIR:-/tmp}/bsdrunner-network-resolv.XXXXXX")"
    search_domain="$(network_search)"
    servers="$(network_nameservers)"
    ...
    if [ -z "$servers" ]; then
        rm -f "$tmp"
        printf 'No non-local network DNS servers were found in resolvconf state.\n'
        return 1
    fi

    mkdir -p "$state_dir"
    if [ -r /etc/resolv.conf ]; then
        cp -p /etc/resolv.conf "$resolv_backup_file"
        printf 'Backed up /etc/resolv.conf to %s.\n' "$resolv_backup_file"
    fi

    {
        printf '# Generated by BSDRunner DNS Cache travel mode.\n'
        printf '# Use the DNS GUI Local Cache action to restore local_unbound.\n'
    ...
    run_privileged install -m 0644 "$tmp" /etc/resolv.conf
    rm -f "$tmp"
    printf 'Installed network DNS servers from resolvconf state.\n'
}
...
emit_snapshot() {
    ...
    search_domain="$(resolv_search)"
    network_search_domain="$(network_search)"
    ...
    printf '"resolver":{"local_active":%s,"encrypted_forwarding":%s,"ca_bundle":"%s","ca_bundle_available":%s,"search":"%s","network_search":"%s","nameservers":' \
        ...
        "$(printf '%s' "$network_search_domain" | json_escape)"
    nameservers_json
    printf ',"network_nameservers":'
    network_nameservers_json
    printf ',"forwarders":'
    forwarders_json
    printf '},'
    printf '"tools":{"drill":%s,"local_unbound_setup":%s,"local_unbound_control":%s,"resolvconf":%s},' \
        ...
        "$(if command -v resolvconf >/dev/null 2>&1; then printf true; else printf false; fi)"
...
do_network_dns() {
    if output="$(write_network_resolv_conf 2>&1)"; then
        write_last_result "warning" "Using network DNS for captive portal or travel."
        emit_action_result true "Using network DNS for captive portal or travel." "$output"
    else
        write_last_result "error" "Unable to switch to network DNS."
        emit_action_result false "Unable to switch to network DNS." "$output"
        exit 1
    fi
}
...
do_local_dns() {
    if command -v local-unbound-setup >/dev/null 2>&1; then
        if output="$(run_privileged sysrc local_unbound_enable=YES 2>&1 && run_privileged local-unbound-setup 2>&1 && run_privileged service local_unbound restart 2>&1)"; then
            write_last_result "success" "Local DNS cache restored."
            emit_action_result true "Local DNS cache restored." "$output"
        else
            write_last_result "error" "Unable to restore local DNS cache."
            emit_action_result false "Unable to restore local DNS cache." "$output"
            exit 1
        fi
        return
    fi

    tmp="$(mktemp "${TMPDIR:-/tmp}/bsdrunner-local-resolv.XXXXXX")"
    {
        printf '# Generated by BSDRunner DNS Cache.\n'
        printf 'nameserver 127.0.0.1\n'
        printf 'options edns0\n'
    } >"$tmp"

    if output="$(run_privileged install -m 0644 "$tmp" /etc/resolv.conf 2>&1 && run_privileged sysrc local_unbound_enable=YES 2>&1 && run_privileged service local_unbound restart 2>&1)"; then
        rm -f "$tmp"
        write_last_result "success" "Local DNS cache restored."
        emit_action_result true "Local DNS cache restored." "$output"
    else
