...
If `security/endlessh` is installed, the GUI also exposes a `Tarpit` setting after LAN SSH is enabled. Enabling tarpit mode chooses and stores a random real SSH port between `22201` and `22299`; applying that profile moves real `sshd` to the stored high port, starts `endlessh` on port `22`, and reloads PF with port `22` routed to the tarpit. Because port `22` is privileged, the backend sets `endlessh_daemonuser=root` while tarpit mode is enabled and restores `nobody` when it is disabled. In tarpit mode the real LAN SSH rule allows fifty active SSH connections per LAN source and four hundred new attempts in 24 hours.
